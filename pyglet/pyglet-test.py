...
def on_key_press(symbol, modifiers):
    ...
    if symbol == key.A:
        ...
        if cir_x > cir_radius: cir_x -= 15
    elif symbol == key.RIGHT:
        if cir_x < window.width - cir_radius: cir_x += 15
    elif symbol == key.DOWN:
        if cir_y > cir_radius: cir_y -= 15
    elif symbol == key.UP:
        if cir_y < window.height - cir_radius: cir_y += 15
    elif symbol == key.C:
...
def on_draw():
    ...
    label.draw()
    # Shapes
    circle = pyglet.shapes.Circle(x=cir_x, y=cir_y, radius=cir_radius, color=(c_R, c_G, c_B))
    circle.draw()
